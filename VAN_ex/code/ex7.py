...
def q5_plots(keyframes, rel_poses, rel_covs, accepted):
    ...
    # ----- (0) Loop-closure plot — side-by-side pre-LC vs post-LC route,
    #            each with LC keyframes marked and straight lines drawn
    #            between paired LC keyframes. Pre-LC chords are long
    #            (they show the drift the LCs are correcting); post-LC
    #            chords collapse to near-zero because the optimiser has
    #            pulled the pairs onto each other.
    # First-visit side of each pair is the earlier keyframe (c_i); the
    # revisit side is the later keyframe (c_n). A KF that appears on both
    # sides for different LCs is classed as a revisit (it's a returned-to
    # location that was itself seen once).
    revisit_kfs = sorted({rec['n'] for rec in accepted})
    first_visit_kfs = sorted({rec['i'] for rec in accepted}
                             - set(revisit_kfs))

    def _draw_lc_panel(ax, cent, title):
        ax.plot(cent[:, 0], cent[:, 2], '-', color='steelblue',
                linewidth=1.0, alpha=0.6, zorder=1, label='trajectory')
        ax.scatter(cent[:, 0], cent[:, 2], s=6, c='steelblue',
                   edgecolor='black', linewidth=0.2, zorder=2,
                   label=f'keyframes (n={len(keyframes)})')
        # c_0 start marker sits UNDER the LC-KF markers so, when c_0 is
        # itself a first-visit LC keyframe, the LC-KF color still shows.
        ax.scatter([0], [0], c='black', s=90, marker='s', zorder=3,
                   label='c_0')
        # LC-keyframe markers ON TOP of the trajectory + c_0 square.
        if first_visit_kfs:
            pts = cent[first_visit_kfs]
            ax.scatter(pts[:, 0], pts[:, 2], s=55, c='seagreen',
                       edgecolor='black', linewidth=0.5, zorder=5,
                       label=f'first-visit LC KFs ({len(first_visit_kfs)})')
        if revisit_kfs:
            pts = cent[revisit_kfs]
            ax.scatter(pts[:, 0], pts[:, 2], s=55, c='crimson',
                       edgecolor='black', linewidth=0.5, zorder=5,
                       label=f'revisit LC KFs ({len(revisit_kfs)})')
        # Chord lines: keyframe → its LC-pair keyframe. Drawn LAST at the
        # highest z-order so they're always visible on top of the markers.
        for rec in accepted:
            x0, z0 = cent[rec['n'], 0], cent[rec['n'], 2]
            x1, z1 = cent[rec['i'], 0], cent[rec['i'], 2]
            ax.plot([x0, x1], [z0, z1], '-', color='black',
                    linewidth=2.2, alpha=1.0, zorder=10)
    ...
    _draw_lc_panel(axes[0], cent_no,
                   f'pre-LC route  ({len(accepted)} pairs, chords = drift)')
    _draw_lc_panel(axes[1], cent_with,
                   f'post-LC route  ({len(accepted)} pairs, chords collapsed)')
    # Share axis limits so the two panels are directly comparable.
    all_x = np.concatenate([cent_no[:, 0], cent_with[:, 0]])
    all_z = np.concatenate([cent_no[:, 2], cent_with[:, 2]])
    pad = 0.05 * max(all_x.ptp(), all_z.ptp())
    for ax in axes:
        ax.set_xlim(all_x.min() - pad, all_x.max() + pad)
        ax.set_ylim(all_z.min() - pad, all_z.max() + pad)
    fig.suptitle(f'Q7.5 — loop-closure pairs on the trajectory  '
                 f'(before vs after optimisation)')
    fig.tight_layout()
    _save(fig, 'q7_5_lc_chords')
